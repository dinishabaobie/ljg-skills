...
读取步骤 A 生成的 markdown 文件，调用 Skill tool 执行 `ljg-card`（默认 `-v`，或按用户指定的模具参数），以 markdown 文件内容为输入。等待完成，获得 PNG 文件路径。
